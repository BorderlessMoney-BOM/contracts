...
async function main() {
  ...
  const args = [usdc, vault, sgRouter, pool];

  const Contract = await hre.ethers.getContractFactory("UnipilotLPStrategy");
  const contract = await Contract.deploy(...args);

  console.log(
    "Strategy deployed to:",
    contract.address,
    `npx hardhat verify ${contract.address} --network matic ` + args.join(" ")
  );
}
